...
def check_hand_value(hand):
    ...
    for card in deck:
        card_value = sum(card[0])# get the card's value here
    ...
    if type(card_value)== int:          # First check if we have 0 aces, in which case we do nothing
        return hand_value
    elif hand_value > 21: # Check if counting the first ace as 11 and all other aces as 1 puts the hand value over 21 (means all aces should be 1)
        hand_value += (1*num_ace) # Add the value of the aces to the hand, in this case they all have a value of 1
        return hand_value
    else:               # The else runs if the first ace has a value of 11 and the rest of the aces have a value of 1 without going over 21
        hand_value += (11+1*(num_ace-1)) # Add the value of the aces to the hand, the first ace counts as 11 and the rest are 1
        return hand_value
# End of Step 3 ---------------------------------------------------------------------------------------------------------------------------------
...
def show_hand_unhidden_cards(hand):
    ...
    for i in range(1,len(hand)+1):
        hand_graphics_unhidden.append(i)                 # append to the list of card graphics here
    ...
    for i in range(7):
        ...
        for j in range(len(hand)):    # The second loop should run for however many cards we have in the hand
            card_str += hand_graphics_unhidden[i][j] + " " # fill in the indexes HINT: use i or j here
...
def show_hand_hidden_cards(hand):
    ...
    for i in range(1, len(hand)+1):                      # This loop should skip the first card in the hand but cover the rest of the cards
        hand_graphics_hidden.append(i)         # Add the back of the cards to our list here
    ...
    for i in range(7):
        card_str = ""     # initialize an empty string to add card graphics to
        for j in range(len(hand)): # The second loop should run for however many cards we have in the hand
            card_str += hand_graphics_hidden[i][j] + " " # fill in the indexes HINT: use i or j here
        print(card_str)
# End of Step 4 ---------------------------------------------------------------------------------------------------------------------------------
...
def dealer_turn(deck, hand):
    ...
    print("Dealer's hand:")
    show_hand_unhidden_cards(hand)# On this line, show the dealer's unhidden hand using the function from step 4
    while sum(hand[0]) < 17: # Keep looping for as long as the value of the hand is less than what the dealer stands on
    ...
    if _sum(dealer_hand[0]) > 21: # Check if the dealer's hand has a value over 21, which means they bust (lose)
        ...
        return sum(hand[0]) # return the final value of the hand
